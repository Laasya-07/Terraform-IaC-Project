...
resource "azurerm_storage_account" "strg_ams" {
  name                     = "strgams${var.environment}reportfn"
...
resource "azurerm_service_plan" "asp" {
  name                = var.report_fnapp_name
  resource_group_name = data.azurerm_resource_group.rg.name
  location            = data.azurerm_resource_group.rg.location
  os_type             = "Linux"
  sku_name            = var.report_asp_sku_name
  tags = {
    Project = var.project_name
    Environment = var.environment
    Owner = var.owners
...
resource "azurerm_linux_function_app" "fnapp" {
  name                = "reports-fnapp-ams-${var.environment}"
  resource_group_name = data.azurerm_resource_group.rg.name
  location            = data.azurerm_resource_group.rg.location
  ...
  app_settings = {
    ...
    ReportSASTokenExpiryMinutes = 2880
